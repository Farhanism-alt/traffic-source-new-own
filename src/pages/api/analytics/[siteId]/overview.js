...
async function syncPaymentsForSite(siteId) {
  ...
  try { const { syncDodoPayments } = await import('@/lib/dodo-sync'); await syncDodoPayments(); } catch {}
  try { const { syncStripePayments } = await import('@/lib/stripe-sync'); await syncStripePayments(); } catch {}
  try { const { syncLemonSqueezyPayments } = await import('@/lib/lemonsqueezy-sync'); await syncLemonSqueezyPayments(); } catch {}
}
...
function buildSessionFilters(query, alias = '') {
  ...
  if (query.entry_page) { clauses.push(`${pfx}entry_page = ?`); params.push(query.entry_page); }
  if (query.exit_page) { clauses.push(`${pfx}exit_page = ?`); params.push(query.exit_page); }
  if (query.browser) { clauses.push(`${pfx}browser = ?`); params.push(query.browser); }
  if (query.os) { clauses.push(`${pfx}os = ?`); params.push(query.os); }
  if (query.device) { clauses.push(`${pfx}device_type = ?`); params.push(query.device); }
  return { clauses, params };
}

function buildPageViewFilters(query) {
  const clauses = [], params = [];
  if (query.page) { clauses.push(`pathname = ?`); params.push(query.page); }
  return { clauses, params };
}

function hasSessionFilters(query) {
  return query.channel || query.country || query.city || query.entry_page || query.exit_page || query.browser || query.os || query.device;
}
...
export default withAuth(async function handler(req, res) {
  ...
  const fromDate = new Date(range.from), toDate = new Date(range.to);
  const daysDiff = Math.ceil((toDate - fromDate) / (1000 * 60 * 60 * 24));
  const prevFrom = new Date(fromDate);
  prevFrom.setDate(prevFrom.getDate() - daysDiff);
  const prevRange = { from: prevFrom.toISOString().slice(0, 10), to: new Date(fromDate.getTime() - 86400000).toISOString().slice(0, 10) };
  ...
  const bounceRate = current.total_sessions > 0 ? ((current.total_bounces / current.total_sessions) * 100).toFixed(1) : 0;
  ...
  if (useSessionFilters || usePageFilter) {
    ...
    if (req.query.period === '24h') {
      timeSeriesPromise = getRows(`SELECT TO_CHAR(timestamp AT TIME ZONE 'UTC', 'YYYY-MM-DD HH24":00"') as date, COUNT(*) as page_views, COUNT(DISTINCT visitor_id) as visitors FROM page_views WHERE site_id = ? AND timestamp >= NOW() - INTERVAL '24 hours' GROUP BY date ORDER BY date ASC`, [siteId]);
    } else {
      timeSeriesPromise = getRows(`SELECT date, visitors, sessions, page_views FROM daily_stats WHERE site_id = ? AND date BETWEEN ? AND ? ORDER BY date ASC`, [siteId, range.from, range.to]);
    }
  }
  ...
  if (useSessionFilters) {
    pagesPromise = getRows(`SELECT pv.pathname as name, COUNT(*) as views, COUNT(DISTINCT pv.visitor_id) as visitors FROM page_views pv INNER JOIN sessions s ON s.site_id = pv.site_id AND s.id = pv.session_id WHERE pv.site_id = ? AND pv.timestamp BETWEEN ? AND ?${sfAliasedWhere}${pvfWhere} GROUP BY pv.pathname ORDER BY views DESC LIMIT 20`, [siteId, range.from, dateEnd, ...sfAliased.params, ...pvf.params]);
  } else {
  ...
  const browsersPromise = sessQ(`site_id = ? AND started_at BETWEEN ? AND ? AND browser IS NOT NULL AND browser != ''`, [siteId, range.from, dateEnd], `SELECT browser as name, COUNT(*) as count`, `GROUP BY browser ORDER BY count DESC LIMIT 10`);
  const osPromise = sessQ(`site_id = ? AND started_at BETWEEN ? AND ? AND os IS NOT NULL AND os != ''`, [siteId, range.from, dateEnd], `SELECT os as name, COUNT(*) as count`, `GROUP BY os ORDER BY count DESC LIMIT 10`);
  const devicesPromise = sessQ(`site_id = ? AND started_at BETWEEN ? AND ? AND device_type IS NOT NULL AND device_type != ''`, [siteId, range.from, dateEnd], `SELECT device_type as name, COUNT(*) as count`, `GROUP BY device_type ORDER BY count DESC LIMIT 10`);

  let convTotalsPromise, convBySourcePromise, convTimeSeriesPromise;
  if (useSessionFilters) {
    convTotalsPromise = getRow(`SELECT COUNT(*) as total_conversions, COALESCE(SUM(CASE WHEN c.currency = 'usd' THEN c.amount ELSE 0 END), 0) as total_revenue, COALESCE(AVG(c.amount), 0) as avg_value FROM conversions c INNER JOIN sessions s ON s.site_id = c.site_id AND s.id = c.session_id WHERE c.site_id = ? AND c.status = 'completed' AND c.created_at BETWEEN ? AND ?${sfAliasedWhere}`, [siteId, range.from, dateEnd, ...sfAliased.params]);
    convBySourcePromise = getRows(`SELECT COALESCE(s.utm_source, s.referrer_domain, 'Direct') as name, COUNT(*) as conversions, SUM(CASE WHEN c.currency = 'usd' THEN c.amount ELSE 0 END) as revenue FROM conversions c INNER JOIN sessions s ON s.site_id = c.site_id AND s.id = c.session_id WHERE c.site_id = ? AND c.status = 'completed' AND c.created_at BETWEEN ? AND ?${sfAliasedWhere} GROUP BY name ORDER BY revenue DESC LIMIT 10`, [siteId, range.from, dateEnd, ...sfAliased.params]);
    convTimeSeriesPromise = getRows(`SELECT DATE(c.created_at) as date, COUNT(*) as conversions, SUM(CASE WHEN c.currency = 'usd' THEN c.amount ELSE 0 END) as revenue FROM conversions c INNER JOIN sessions s ON s.site_id = c.site_id AND s.id = c.session_id WHERE c.site_id = ? AND c.status = 'completed' AND c.created_at BETWEEN ? AND ?${sfAliasedWhere} GROUP BY date ORDER BY date ASC`, [siteId, range.from, dateEnd, ...sfAliased.params]);
  } else {
    convTotalsPromise = getRow(`SELECT COUNT(*) as total_conversions, COALESCE(SUM(CASE WHEN currency = 'usd' THEN amount ELSE 0 END), 0) as total_revenue, COALESCE(AVG(amount), 0) as avg_value FROM conversions WHERE site_id = ? AND status = 'completed' AND created_at BETWEEN ? AND ?`, [siteId, range.from, dateEnd]);
    convBySourcePromise = getRows(`SELECT COALESCE(utm_source, referrer_domain, 'Direct') as name, COUNT(*) as conversions, SUM(CASE WHEN currency = 'usd' THEN amount ELSE 0 END) as revenue FROM conversions WHERE site_id = ? AND status = 'completed' AND created_at BETWEEN ? AND ? GROUP BY name ORDER BY revenue DESC LIMIT 10`, [siteId, range.from, dateEnd]);
    convTimeSeriesPromise = getRows(`SELECT DATE(created_at) as date, COUNT(*) as conversions, SUM(CASE WHEN currency = 'usd' THEN amount ELSE 0 END) as revenue FROM conversions WHERE site_id = ? AND status = 'completed' AND created_at BETWEEN ? AND ? GROUP BY date ORDER BY date ASC`, [siteId, range.from, dateEnd]);
  }
